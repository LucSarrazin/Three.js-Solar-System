...
loader.load( '/kawai/scene.gltf', function ( gltf ) {
  ...
  gltf.scene.traverse((child) => {
    if (child.isMesh) {
      child.name = "Kawai"; // Set a name for the kawai model
    }
  });
...
if (infoBoxMoon && infoBoxMoon.style.display === "block") {
  const screenPos = toScreenPosition(sphere, camera);
  const boxWidth = infoBoxMoon.offsetWidth;
  const boxHeight = infoBoxMoon.offsetHeight;
  ...
  if (isVisible) {
    ...
    infoBoxMoon.style.display = "none";
  }
}

  
  const infoBoxKawai = document.getElementById("infoBoxKawai");
  if (infoBoxKawai && infoBoxKawai.style.display === "block" && Kawai) {
    const screenPos = toScreenPosition(Kawai, camera);
    const boxWidth = infoBoxKawai.offsetWidth;
    const boxHeight = infoBoxKawai.offsetHeight;
    ...
    if (isVisible) {
      ...
      infoBoxKawai.style.display = "none";
    }
  }
